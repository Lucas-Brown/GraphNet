...
public class GeneticAlgorithmCyclicalTest {
    ...
    public static void main(String[] args)
    {
        ...
        GeneticTrainer gt = new GeneticTrainer(100, 1000, GeneticAlgorithmCyclicalTest::cyclicalFitness);
        ...
        best.setInputOperation(input_nodes -> input_nodes.get(input_id).acceptUserForwardSignal(0));
        best.setOutputOperation(output_nodes -> {});

        best.inferenceStep();
        best.setInputOperation(input_nodes -> {});
    ...
    private static double cyclicalFitness(IGeneticTrainable individual)
    {
        ...
        net.setInputOperation(input_nodes -> input_nodes.get(input_id).acceptUserForwardSignal(0));
        net.setOutputOperation(output_nodes -> outputs.add(output_nodes.get(output_id).getValueOrNull()));

        // accept the single incoming signal
        net.inferenceStep();
        ...
        // test the network
        for (int i = 1; i < collection_points; i++) {
            net.inferenceStep();
        }
        ...
        score += (null_counts-2)*null_penalty; 
        return score;
    }
